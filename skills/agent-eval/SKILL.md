...
7. **Be honest about sample size — and don't just say so, measure it.** With under ~20 cases, a 2-3 case swing can look like a large percentage shift. `--ci` reports a 95% bootstrap confidence interval on pass rate and mean score, and — with `--baseline` — a paired significance test on both the pass-rate delta and the raw mean-score delta between the two runs:
   ...
   The CI's *significance* verdict and its *width* aren't the same claim, and only one of them is reliable at small n. A narrow bootstrap CI at, say, n=3 isn't reassuring — it's often just an artifact of resampling a tiny, low-variance sample, not evidence the estimate is well pinned down. Confirmed empirically: mean CI width barely narrows from n=3 (~0.55 of the full range) to n=8-10 (~0.5), only dropping meaningfully past n=30-50. `--ci` now prints an explicit ⚠ next to any CI (run-wide or per-category) whose n is below 20 — reusing the same 20-case threshold as the sample-size warning above rather than inventing a second cutoff. The p-value next to it can still be trusted (that's what `min_n` in the next paragraph is for); the interval's *width* specifically can't. This warning shipped print-only at first — anything reading `--json-out` instead of the terminal (a dashboard, a script comparing runs) had no way to reconstruct it short of hardcoding the same threshold itself. Every CI and paired-diff dict now also carries a `"ci_width_reliable"` boolean in the JSON, derived from the identical comparison, so a machine consumer gets the same signal without duplicating the constant.
   ...
   Running that many tests together has its own cost: at a plain alpha=0.05 each, the chance that at least one comes back "significant" purely by chance climbs well past 5% as more tests are added — confirmed empirically at ~27.5% on genuinely stable data across 4 categories, not a theoretical worry. Every verdict — run-wide and per-category — is corrected for however many tests actually ran together, bringing that back to ~8%, close to the nominal target, via the **Benjamini-Hochberg (FDR)** procedure. An earlier version of this used the simpler Bonferroni correction instead; it was replaced after a second, independently confirmed problem: fixing one real regression's data and only varying how many *unrelated*, entirely stable categories existed alongside it, Bonferroni flipped that same regression from detected to undetected once 3+ unrelated categories were added — a real eval set's detection power silently depending on how many categories it happens to have. Benjamini-Hochberg kept detecting it through 7 unrelated categories in the same test, with the false-positive rate just as controlled. See [`bootstrap_stats.py`](./scripts/bootstrap_stats.py) and `apply_multiple_comparisons_correction()`'s docstring in `score_eval.py` for the confirmed numbers; `bonferroni_alpha()` is kept as a stricter, still-available alternative.

   A third fix on the same feature, same discipline: regression candidates (`diff < 0`) and everything else (improvements, and the exact-zero-diff case) are corrected as **two separate** Benjamini-Hochberg families, not one pooled ranking. Confirmed necessary, not a style choice — pooling them let an unrelated, genuinely significant *improvement* elsewhere in the same run lower the effective bar for a borderline regression to pass (BH's threshold grows with rank, and low-p-value improvements occupy the earliest ranks ahead of it), even though only regression candidates can ever fire `--fail-on-significant-regression`. Direct, confound-free check: a fixed regression's significance now stays identical regardless of how many unrelated improvement (or stable) diffs are added alongside it. Improvements still get their own BH-corrected significance for reporting — "SIGNIFICANT IMPROVEMENT" stays a real, corrected claim — just never at the regression family's expense.
