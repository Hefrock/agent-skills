...
4. **Once priorities are declared**, narrow among flaggable projects: sort by `priority` (high → medium → low), then by `days_since(updated) ÷ checkin_interval` as a tiebreaker within a tier. Surface the top item. Include a second only if it shares the same `priority` tier as the first *and* its overdue-ratio is within 15% of it — a real tie, not a forced pair. Cap at 2, always.
...
5. **Before presenting anything:** if the observation spans more than one identity compartment (per whatever compartments the vault's own privacy-threat-modeling concept defines), say so explicitly, plainly, as part of the output — never let a cross-compartment throughline pass as if it were compartment-neutral.
