...
python skills/broadcast/scripts/orchestrate.py --data-dir ~/.broadcast-data \
  ...
  [--host-style single|dialogue]         # default: single — see below
```

**`--host-style dialogue`** adds a second voice, Host B, reacting to each story alongside Host A's existing grounded narration — closer to a two-host conversational feel (à la NotebookLM's "Deep Dive") than today's single-narrator default. The key design choice: **Host B's words are never Gemini-generated.** Every reactive turn is picked from a small, fixed, hand-authored pool (`narrate.REACTIVE_TEMPLATES`) — zero network calls, zero hallucination surface — so this mode adds no new claim that needs grounding; it's a presentation layer over the exact same single grounded narration `--host-style single` already produces, not a second source of prose to verify. `qa_gate.py`'s `reactive_turns_within_pool` check mechanically confirms every reactive turn actually came from that pool, not trusted by its own label. Cost impact is small — a few extra spoken seconds per story via `audio_synth.synthesize_dialogue()`'s two-voice TTS call, not a new class of Gemini call. A story that fails narration's grounding check (or when `--host-style single`) never gets a second voice at all — always exactly today's single mechanical/narrated text, never a partial dialogue.
...
3. **`ingest_failed` non-empty** — a source documented in `config/sources.json` as `feed_url_verified: false` failing is expected every run; check the registry entry's `feed_url_verified_note` first. Any *other* source failing is new: check the error string — a previously-working source returning a *different* error than before usually means the feed URL or endpoint changed upstream, not a bug in this pipeline.
