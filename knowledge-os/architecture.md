...
These two, plus the `Maps/_context.md` hot cache above, are **system files**: exempt from constitution Laws 6 (the connectivity half — "mature" requiring ≥2 links), 7 (no islands), and 8 (provenance). They are read by path rather than by `[[wikilink]]`, so zero inbound links is correct rather than broken, a mechanically regenerated file has no prompting journal entry to cite, and `status: mature` on one means "stable," not "well-connected." See the constitution's "Scope of Laws 6, 7, and 8". Hand-authored `Maps/` pages like `Maps/AI.md` are ordinary notes and stay in scope for all three.
