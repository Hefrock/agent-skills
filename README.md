...
| [`wiki-teacher`](./skills/wiki-teacher) | Knowledge Management | Accountability and growth-mindset system for several concurrent projects — `/checkin` (portfolio-aware accountability, a forcing function, batch-elicits priority for all overdue projects at once, answers "what should I work on" even when nothing's overdue, reports portfolio breadth), `/teach` (project-scoped teaching, extends `/quiz`), `/reflect` (user-initiated, portfolio-wide throughline-spotting — including breadth patterns like nothing reaching `complete` in a while — backed by a structural compartment-privacy check). Stateless — priority/checkin_interval/status/compartment live in each project's own frontmatter. `scripts/wiki_teacher.py`, 35-test regression suite. Requires Obsidian MCP connected. |
...
│   ├── wiki-teacher/           # /checkin /teach /reflect — scripts/wiki_teacher.py, 35-test regression suite
